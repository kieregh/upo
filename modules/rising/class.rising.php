...
class rising {
	...
	public function postlisting($pageNo){
		...
		$selRes="SELECT A.*, COUNT(tbl_comment.postid) as commentcount, B.refId, B.refType, B.voteType, tbl_categories.categoryName, tbl_users.username, A.embeddcode, COUNT(B.id) AS totalVotes,SUM(IF(B.voteType='u',1,0)) as avgVotes FROM tbl_post AS A INNER JOIN tbl_categories ON (tbl_categories.id = A.catId) LEFT JOIN tbl_comment ON (A.id=tbl_comment.postid) INNER JOIN tbl_users ON (A.uid = tbl_users.id) LEFT JOIN `tbl_votes` AS B ON A.id=B.refId AND B.refType ='p' AND voteType ='u' where A.isActive='y' GROUP BY A.id ORDER BY avgVotes desc";
